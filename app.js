(() => {
  ...
  function drawCadWorkspace() {
    ...
    for(const obj of state.objects){if(obj.type==='wall')drawWallCad(obj,obj.id===state.selectedObjectId?sel:obj.id===state.hoveredObjectId?hover:line);}
  ...
  function dimensionGeometry(obj) {
    if (obj.p1 && obj.p2) {
      ...
      return {p1,p2,d1:{x:p1.x+nx*off,y:p1.y+ny*off},d2:{x:p2.x+nx*off,y:p2.y+ny*off},nx,ny,len,offset:off};
    }
    if (obj.a && obj.b) {
      const p1=obj.a,p2=obj.b,v={x:p2.x-p1.x,y:p2.y-p1.y},len=Math.max(.000001,Math.hypot(v.x,v.y));
      return {p1,p2,d1:p1,d2:p2,nx:-v.y/len,ny:v.x/len,len,offset:0};
    }
    return null;
  }
  ...
  function drawOpeningHandles(obj,g){
    if(obj.id!==state.selectedObjectId)return;const color=getComputedStyle(document.documentElement).getPropertyValue('--selection').trim();const bg=getComputedStyle(document.documentElement).getPropertyValue('--canvas').trim();ctx.save();ctx.fillStyle=color;ctx.strokeStyle=bg;ctx.lineWidth=1.5;for(const p of[toScreenCss(g.p1),toScreenCss(g.p2)]){ctx.beginPath();ctx.rect(p.x-4,p.y-4,8,8);ctx.fill();ctx.stroke();}const c=toScreenCss(g.center);ctx.beginPath();ctx.arc(c.x,c.y,5,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.restore();
  }

  function drawDimensionHandles(obj,g){
    const color=getComputedStyle(document.documentElement).getPropertyValue('--selection').trim(),bg=getComputedStyle(document.documentElement).getPropertyValue('--canvas').trim();ctx.save();ctx.fillStyle=color;ctx.strokeStyle=bg;ctx.lineWidth=1.5;for(const p of[toScreenCss(g.p1),toScreenCss(g.p2)]){ctx.beginPath();ctx.arc(p.x,p.y,4.5,0,Math.PI*2);ctx.fill();ctx.stroke();}const c=toScreenCss({x:(g.d1.x+g.d2.x)/2,y:(g.d1.y+g.d2.y)/2});ctx.beginPath();ctx.moveTo(c.x,c.y-5);ctx.lineTo(c.x+5,c.y);ctx.lineTo(c.x,c.y+5);ctx.lineTo(c.x-5,c.y);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();
  }
  ...
  function undo(){if(!state.history.length)return;state.future.push(JSON.stringify(state.objects));state.objects=JSON.parse(state.history.pop());state.selectedObjectId=null;markDirty(true);rebuildObjectSnapIndex();updateAll();}
  function redo(){if(!state.future.length)return;state.history.push(JSON.stringify(state.objects));state.objects=JSON.parse(state.future.pop());state.selectedObjectId=null;markDirty(true);rebuildObjectSnapIndex();updateAll();}
  function updateUndoRedo(){dom.undoBtn.disabled=!state.history.length;dom.redoBtn.disabled=!state.future.length;}

  function commitSegment(a,b,type){if(distance(a,b)<.001)return;pushHistory();let obj;if(type==='wall'){obj={id:uid('wall'),type:'wall',layerId:'walls',a:{...a},b:{...b},thickness:currentWallThickness()};}else{obj={id:uid('cadLine'),type:state.toolset==='cad'?'cadLine':'line',cadLayer:state.activeCadLayer||'0',layerId:'drawing',a:{...a},b:{...b}};if(obj.type==='cadLine'&&!state.cadLayerVisibility.has(obj.cadLayer))state.cadLayerVisibility.set(obj.cadLayer,true);}state.objects.push(obj);state.selectedObjectId=obj.id;state.drawStart={...b};state.previewEnd={...b};markDirty(true);rebuildObjectSnapIndex();updateAll();}
  function commitMeasurement(a,b){const len=distance(a,b);if(len<.001)return;pushHistory();const obj={id:uid('dimension'),type:'dimension',layerId:'dimensions',p1:{...a},p2:{...b},offset:Math.max(250,Math.min(600,len*.08))};state.objects.push(obj);state.selectedObjectId=obj.id;state.measureStart=null;state.previewEnd=null;markDirty(true);rebuildObjectSnapIndex();updateAll();}
  ...
  function collectObjectSnapPoints(){const pts=[];for(const o of state.objects){if(o.a)pts.push({x:o.a.x,y:o.a.y,objectId:o.id});if(o.b)pts.push({x:o.b.x,y:o.b.y,objectId:o.id});if(o.center)pts.push({x:o.center.x,y:o.center.y,objectId:o.id});if(o.point)pts.push({x:o.point.x,y:o.point.y,objectId:o.id});if(o.type==='door'||o.type==='window'){const g=openingGeometry(o);if(g){pts.push({...g.p1,objectId:o.id},{...g.p2,objectId:o.id},{...g.center,objectId:o.id});}}if(o.type==='dimension'){const g=dimensionGeometry(o);if(g)pts.push({...g.p1,objectId:o.id},{...g.p2,objectId:o.id});}}return pts;}
  ...
  function nearestSnap(p,excludeObjectId=null){if(!state.snap)return p;const threshold=10/state.camera.zoom;let best=null,bestD=threshold;const test=q=>{if(excludeObjectId&&q.objectId===excludeObjectId)return;const d=distance(p,q);if(d<bestD){bestD=d;best=q;}};if(state.objectSnapIndex)queryObjectSnapIndex(p,threshold,test);else for(const o of state.objects){if(o.a)test(o.a);if(o.b)test(o.b);}
  ...
  function hitHandle(p,obj){const tol=9/state.camera.zoom;if(!obj)return null;if((obj.type==='wall'||obj.type==='line'||obj.type==='cadLine')&&obj.a&&obj.b){if(distance(p,obj.a)<=tol)return'a';if(distance(p,obj.b)<=tol)return'b';}
    ...
    if(obj.type==='dimension'){const g=dimensionGeometry(obj);if(g){if(distance(p,g.p1)<=tol)return'p1';if(distance(p,g.p2)<=tol)return'p2';const c={x:(g.d1.x+g.d2.x)/2,y:(g.d1.y+g.d2.y)/2};if(distance(p,c)<=tol)return'offset';}}
    return null;}
  function objectBodyDistance(p,o){if(o.type==='door'){const g=doorGeometry(o);return g?Math.min(pointSegmentDistance(p,g.p1,g.p2),pointSegmentDistance(p,g.hinge,g.leafEnd)):Infinity;}if(o.type==='window'){const g=openingGeometry(o);return g?pointSegmentDistance(p,g.p1,g.p2):Infinity;}if(o.type==='dimension'){const g=dimensionGeometry(o);return g?pointSegmentDistance(p,g.d1,g.d2):Infinity;}if(o.type==='cadCircle')return Math.abs(distance(p,o.center)-o.radius);if(o.a&&o.b){let d=pointSegmentDistance(p,o.a,o.b);if(o.type==='wall')d=Math.max(0,d-(o.thickness||150)/2);return d;}return Infinity;}
  function hitObject(p){const tolerance=9/state.camera.zoom;let best=null,bestD=Infinity;const semanticOnly=state.toolset==='plan';for(let i=state.objects.length-1;i>=0;i--){const o=state.objects[i];if(semanticOnly&&isCadObject(o))continue;if(state.toolset==='cad'&&isCadObject(o)&&!cadLayerVisible(o.cadLayer))continue;const d=objectBodyDistance(p,o);if(d<tolerance&&d<bestD){best=o;bestD=d;}}return best;}
  function pointSegmentDistance(p,a,b){return projectPointToSegment(p,a,b).distance;}

  function beginObjectDrag(e,p,obj){const handle=hitHandle(p,obj);const mode=handle||'body';state.dragEdit={pointerId:e.pointerId,objectId:obj.id,mode,start:{...p},snapshot:JSON.parse(JSON.stringify(obj)),historyPushed:false};canvas.setPointerCapture?.(e.pointerId);host.dataset.drag='true';}
  ...
  function applyObjectDrag(p){const d=state.dragEdit;if(!d)return;const obj=state.objects.find(o=>o.id===d.objectId);if(!obj)return;const src=d.snapshot,delta={x:p.x-d.start.x,y:p.y-d.start.y};ensureDragHistory();
    if((obj.type==='wall'||obj.type==='line'||obj.type==='cadLine')&&src.a&&src.b){if(d.mode==='a')obj.a={...p};else if(d.mode==='b')obj.b={...p};else{obj.a={x:src.a.x+delta.x,y:src.a.y+delta.y};obj.b={x:src.b.x+delta.x,y:src.b.y+delta.y};}}
    else if(obj.type==='door'||obj.type==='window'){const wall=state.objects.find(o=>o.id===obj.wallId&&o.type==='wall');if(wall){const pr=projectPointToSegment(p,wall.a,wall.b);if(d.mode==='center'||d.mode==='body')obj.t=pr.t;else if(d.mode==='p1'||d.mode==='p2'){const g0=openingGeometry(src);if(g0){const opposite=d.mode==='p1'?g0.p2:g0.p1;const pp=projectPointToSegment(p,wall.a,wall.b);const po=projectPointToSegment(opposite,wall.a,wall.b);const len=distance(wall.a,wall.b);obj.t=clamp((pp.t+po.t)/2,0,1);obj.width=Math.max(100,Math.abs(pp.t-po.t)*len);}}}}
    else if(obj.type==='dimension'){const g0=dimensionGeometry(src);if(g0){if(d.mode==='p1')obj.p1={...p};else if(d.mode==='p2')obj.p2={...p};else if(d.mode==='offset'){const base={x:(obj.p1.x+obj.p2.x)/2,y:(obj.p1.y+obj.p2.y)/2},v={x:obj.p2.x-obj.p1.x,y:obj.p2.y-obj.p1.y},len=Math.max(.000001,Math.hypot(v.x,v.y)),nx=-v.y/len,ny=v.x/len;obj.offset=(p.x-base.x)*nx+(p.y-base.y)*ny;}else{obj.p1={x:g0.p1.x+delta.x,y:g0.p1.y+delta.y};obj.p2={x:g0.p2.x+delta.x,y:g0.p2.y+delta.y};}}}
  ...
  function onPointerDown(e){host.focus();const s=fromPointerEvent(e);if(isCompactViewer()){if(e.button===0||e.pointerType==='touch'){e.preventDefault();beginViewerPointer(e,s);}return;}const panGesture=e.button===1||(e.button===0&&state.spaceDown);if(panGesture){e.preventDefault();state.pan={startScreen:s,startCamera:{...state.camera}};host.dataset.pan='true';canvas.setPointerCapture?.(e.pointerId);return;}if(e.button!==0)return;let raw=screenCssToWorld(s),p=nearestSnap(raw);
    ...
    if(state.activeTool==='select'){const selected=state.objects.find(o=>o.id===state.selectedObjectId);if(selected&&hitHandle(raw,selected)){beginObjectDrag(e,raw,selected);return;}const obj=hitObject(raw);state.selectedObjectId=obj?.id||null;state.selectedReferenceId=null;if(obj)beginObjectDrag(e,raw,obj);renderProperties();render();return;}
  ...
  function onPointerUp(e){if(isCompactViewer()&&state.viewerPointers.has(e.pointerId)){endViewerPointer(e);return;}if(state.dragEdit){const hadChange=state.dragEdit.historyPushed;state.dragEdit=null;host.dataset.drag='false';try{canvas.releasePointerCapture?.(e.pointerId);}catch(_){}if(hadChange){rebuildObjectSnapIndex();updateAll();}return;}if(!state.pan)return;state.pan=null;host.dataset.pan='false';try{canvas.releasePointerCapture?.(e.pointerId);}catch(_){} }
  ...
  function renderProperties(){dom.propertiesPanel.innerHTML='';const obj=state.objects.find(o=>o.id===state.selectedObjectId);if(obj){propertyText(t('property.type'),localizedObjectType(obj.type));if(state.toolset==='cad')propertyText(t('property.layer'),cadLayerForObject(obj));
      ...
      if(obj.type==='wall')propertyNumber(t('property.thickness'),obj.thickness,v=>{if(v>0){pushHistory();obj.thickness=v;markDirty(true);updateAll();}},'mm');
      if(obj.type==='door'||obj.type==='window'){propertyNumber(t('property.width'),obj.width,v=>{if(v>0){pushHistory();obj.width=v;markDirty(true);rebuildObjectSnapIndex();updateAll();}},'mm');if(obj.type==='door'){propertyText(t('property.openingSide'),`${obj.hinge==='end'?t('value.hingeEnd'):t('value.hingeStart')} · ${obj.swing===-1?t('value.swingReverse'):t('value.swingNormal')}`);const actions=document.createElement('div');actions.className='property-actions';const hinge=document.createElement('button');hinge.className='property-action';hinge.textContent=t('action.flipHinge');hinge.addEventListener('click',()=>{pushHistory();obj.hinge=obj.hinge==='end'?'start':'end';markDirty(true);updateAll();});const swing=document.createElement('button');swing.className='property-action';swing.textContent=t('action.flipSwing');swing.addEventListener('click',()=>{pushHistory();obj.swing=obj.swing===-1?1:-1;markDirty(true);updateAll();});actions.append(hinge,swing);dom.propertiesPanel.appendChild(actions);}}
      if(obj.type==='dimension'){const g=dimensionGeometry(obj);if(g){propertyText(t('property.length'),`${formatNumber(g.len,2)} mm`);propertyNumber(t('property.offset'),g.offset,v=>{pushHistory();obj.offset=v;markDirty(true);updateAll();},'mm');}}

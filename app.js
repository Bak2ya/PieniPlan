(() => {
  ...
  function ensureSpaceMetadata(){
    ...
    for(const floor of state.floors){
      ...
      for(const space of spaces){
        ...
        if(space.areaMode!=='manual'&&space.areaMode!=='calculated'){space.areaMode=Number.isFinite(Number(space.manualAreaM2))&&Number(space.manualAreaM2)>0?'manual':'calculated';changed=true;}
        if(space.manualAreaM2!=null&&(!Number.isFinite(Number(space.manualAreaM2))||Number(space.manualAreaM2)<=0)){space.manualAreaM2=null;if(space.areaMode==='manual')space.areaMode='calculated';changed=true;}
    ...
    for(const space of state.objects.filter(o=>o.type==='space'&&!o.floorId)){
      space.floorId=state.activeFloorId||state.floors[0]?.id||'floor_1';space.spaceUuid=space.spaceUuid||makeStableUuid();space.spaceType=space.spaceType||'unspecified';space.areaMode=Number.isFinite(Number(space.manualAreaM2))&&Number(space.manualAreaM2)>0?'manual':'calculated';space.manualAreaM2=space.areaMode==='manual'?Number(space.manualAreaM2):null;space.name=space.name||nextSpaceName(space.floorId);changed=true;
    }
    return changed;
  }
  ...
  function usesManualSpaceArea(space){return space?.areaMode==='manual'&&Number.isFinite(Number(space?.manualAreaM2))&&Number(space.manualAreaM2)>0;}
  function displaySpaceAreaM2(space){return usesManualSpaceArea(space)?Number(space.manualAreaM2):calculatedSpaceAreaM2(space);}
  function setSpaceManualArea(space,value){const area=Number(value);if(!space||!Number.isFinite(area)||area<=0)return false;pushHistory();space.areaMode='manual';space.manualAreaM2=area;markSpaceUserEdited(space);markDirty(true);updateAll();return true;}
  function setSpaceCalculatedArea(space){if(!space)return;pushHistory();space.areaMode='calculated';space.manualAreaM2=null;markSpaceUserEdited(space);markDirty(true);updateAll();}
  function isHingedDoorType(type){return ['hingedSingle','hingedDouble','doubleActingSingle','doubleActingDouble'].includes(type||'hingedSingle');}
  ...
  function ensureDoorMetadata(){let changed=false;for(const o of state.objects){if(o.type!=='door')continue;if(o.swingSide!==-1&&o.swingSide!==1){o.swingSide=doorSwingSide(o);changed=true;}syncLegacyDoorSwing(o);}return changed;}
  ...
  function drawOpeningPlan(obj, kind, color) {
    ...
    if(kind==='door'){
      ...
      if(type==='hingedSingle')drawHingedLeaf(obj,g,stroke,0,1,{hinge:obj.hinge||'start'});
  ...
  function detectClosedWallFaces(){
    ...
    return detectFacesFromSegments(segments,{minArea:10000,ignoreInteriorCrossings:false});
  }
  ...
  function dxfDoorEntities(obj,layer){
    ...
    if(type==='hingedSingle')emitLeaf(0,1,obj.hinge||'start');

(() => {
  ...
  const dom = {
    ...
    viewFitAction: $('viewFitAction'), viewAllAction: $('viewAllAction'), fileNewAction: $('fileNewAction'), fileOpenProjectAction: $('fileOpenProjectAction'), fileSaveProjectAction: $('fileSaveProjectAction'), fileOpenDxfAction: $('fileOpenDxfAction'), fileExportDxfAction: $('fileExportDxfAction'), helpBtn: $('helpBtn'), aboutBtn: $('aboutBtn'), helpBackdrop: $('helpBackdrop'), aboutBackdrop: $('aboutBackdrop'), helpCloseBtn: $('helpCloseBtn'), aboutCloseBtn: $('aboutCloseBtn'), helpContent: $('helpContent'), aboutVersion: $('aboutVersion'),
  ...
  const toolIconFallback = { select:'select', constraint:'vector', region:'vector', line:'line', wall:'wall', door:'door', window:'window', space:'space', measure:'dimension', trim:'modify', extend:'modify', delete:'modify' };
  ...
  function helpHtml(section){const ko=i18n.language==='ko';const data={
    ...
    files:ko?`<h2>파일과 프로젝트</h2><ul><li><strong>새 도면</strong>: 현재 작업을 초기화합니다.</li><li><strong>프로젝트 열기/저장</strong>: .pieniplan 파일에 층, Plan/CAD 객체, 레이어, 참조, 카메라와 인식 이력을 저장합니다. 지원 브라우저에서는 처음 선택한 프로젝트 파일을 이후 저장에서 직접 갱신하며, 직접 파일 쓰기를 지원하지 않는 브라우저에서만 다운로드 방식으로 대체합니다.</li><li><strong>DXF 열기</strong>: CAD Mode에서 편집 가능한 DXF를 엽니다.</li><li><strong>DXF 내보내기</strong>: 현재 도면을 DXF로 생성합니다.</li></ul><p>원본 DXF는 자동으로 덮어쓰지 않습니다.</p>`:`<h2>Files & projects</h2><p>.pieniplan stores the editable project state. On browsers with direct file access, the chosen project file is updated in place on later saves; other browsers fall back to a download. DXF export is separate and never silently overwrites the source DXF.</p>`,
    shortcuts:ko?`<h2>단축키</h2><table class="help-shortcuts"><tr><td><kbd>Ctrl/Cmd+Z</kbd></td><td>실행 취소</td></tr><tr><td><kbd>Ctrl/Cmd+Shift+Z</kbd></td><td>다시 실행</td></tr><tr><td><kbd>Ctrl/Cmd+S</kbd></td><td>프로젝트 저장</td></tr><tr><td><kbd>Delete</kbd></td><td>선택 객체 삭제</td></tr><tr><td><kbd>Esc</kbd></td><td>현재 작업 취소 / 선택 해제</td></tr><tr><td><kbd>F3</kbd></td><td>SNAP</td></tr><tr><td><kbd>F7</kbd></td><td>GRID</td></tr><tr><td><kbd>F8</kbd></td><td>ORTHO</td></tr><tr><td><kbd>F10</kbd></td><td>POLAR</td></tr><tr><td><kbd>L</kbd></td><td>LINE</td></tr><tr><td><kbd>TR</kbd> / <kbd>EX</kbd></td><td>TRIM / EXTEND</td></tr><tr><td><kbd>S</kbd></td><td>STRETCH 예약(현재 미지원) · 저장은 Ctrl/Cmd+S</td></tr></table>`:`<h2>Shortcuts</h2><table class="help-shortcuts"><tr><td><kbd>Ctrl/Cmd+Z</kbd></td><td>Undo</td></tr><tr><td><kbd>Ctrl/Cmd+Shift+Z</kbd></td><td>Redo</td></tr><tr><td><kbd>Ctrl/Cmd+S</kbd></td><td>Save project</td></tr><tr><td><kbd>F3/F7/F8/F10</kbd></td><td>SNAP / GRID / ORTHO / POLAR</td></tr><tr><td><kbd>S</kbd></td><td>Reserved for STRETCH (not implemented yet); save with Ctrl/Cmd+S</td></tr></table>`,
  ...
  function getLinkedCadRenderCache(ref,region){
    ...
    const cache={revision:state.cadRenderRevision,sig,layers};linkedCadRenderCache.set(ref,cache);return cache;
  }
  ...
  function nearestSnap(p,excludeObjectId=null){
    ...
    if(wantsWallProjection){for(const wall of getPlanObjects()){if(wall.type!=='wall'||wall.id===excludeObjectId)continue;const pr=wallProjectPoint(p,wall);if(pr.distance<bestD)test({...pr.point,objectId:wall.id},'wall');}}
  ...
  function onPointerDown(e){host.focus();hideContextMenu();const s=fromPointerEvent(e);if(isCompactViewer()){if(e.button===0||e.pointerType==='touch'){e.preventDefault();beginViewerPointer(e,s);}return;}const panGesture=e.button===1||(e.button===0&&state.spaceDown);if(panGesture){e.preventDefault();if(state.spaceGesture)state.spaceGesture.used=true;state.pan={pointerId:e.pointerId,startScreen:s,startCamera:{...state.camera}};host.dataset.pan='true';canvas.setPointerCapture?.(e.pointerId);return;}if(e.button!==0)return;const editableLabel=hitEditableLabel(s);if(editableLabel)return;let raw=screenCssToWorld(s),p=nearestSnap(raw);
    ...
    if(state.activeTool==='trim'||state.activeTool==='extend'){applyTrimExtendAtPoint(raw,state.activeTool);return;}
    if(state.activeTool==='line'||state.activeTool==='wall'){
  ...
  async function writeProjectToHandle(handle,json,name){const writable=await handle.createWritable();await writable.write(json);await writable.close();state.projectFileHandle=handle;state.projectFileName=handle.name||name;markDirty(false);setCommandStatus(t('project.saved',{name:state.projectFileName}),'strong');}
  async function saveProjectFile({saveAs=false}={}){
    try{
      ...
      downloadBlob(new Blob([json],{type:'application/json;charset=utf-8'}),name);state.projectFileName=name;markDirty(false);setCommandStatus(t('project.downloadFallback',{name}),'strong');
  ...
  function openRegionInPlan(region){let ref=state.references.find(r=>r.type==='linkedCadRegion'&&r.regionId===region.id);if(!ref){const layers=visibleCadLayers(region);ref={id:uid('ref'),type:'linkedCadRegion',regionId:region.id,name:`${region.name} · CAD`,visible:true,opacity:.48,layers:[...layers],visibleLayers:new Set(layers)};state.references.push(ref);}else ref.visible=true;ensureFloorModel();const floor=activeFloor();if(floor&&!floor.sourceRegionId){maybeAdoptFloorNameFromRegion(floor,region);floor.sourceRegionId=region.id;}state.selectedReferenceId=ref.id;markDirty(true);switchToolset('plan',{skipMapping:true});switchInspector('primary');fitBounds(region);updateAll();}
  ...
  function cleanupArchitecturalWallTopology(walls,sourceLines=[]){
    const recovered=recoverArchitecturalWallEdgeExtensions(walls,sourceLines),cornered=snapArchitecturalWallCorners(recovered),connected=snapArchitecturalEndpointsToWalls(cornered),runs=mergeCollinearArchitecturalRuns(connected),trimmed=trimArchitecturalWallOverruns(runs),resnapped=snapArchitecturalEndpointsToWalls(trimmed);return resnapped.filter(w=>w.geometry==='arc'||distance(w.a,w.b)>=250);
  }
  ...
  function detectWallCandidates(region){
    const lines=wallRecognitionSourceLines(region),arcs=wallRecognitionSourceArcs(region),stairs=detectStairCandidates(lines,region),stairIds=new Set(stairs.flatMap(s=>s.sourceIds)),segmentedArcs=detectSegmentedArcCandidates(lines.filter(l=>!stairIds.has(l.id))),segArcIds=new Set(segmentedArcs.flatMap(a=>a.sourceIds||[])),structural=lines.filter(l=>l.length>=280&&!stairIds.has(l.id)&&!segArcIds.has(l.id)),bins=new Map();for(const line of structural){const key=Math.round(line.angle/2)*2;if(!bins.has(key))bins.set(key,[]);bins.get(key).push(line);}const pairs=[];
    ...
    const thicknessClusters=clusterCommonWallThicknesses(pairs),straightWalls=mergeArchitecturalWallBands(pairs.sort((a,b)=>b.score-a.score),thicknessClusters,structural),arcWalls=detectArcWallCandidates(arcs),walls=[...straightWalls,...arcWalls,...segmentedArcs],arcWallSourceIds=new Set(arcWalls.flatMap(w=>w.sourceIds||[])),segments=walls.flatMap(sampleCandidateWallSegments),regionArea=Math.max(1,(region.maxx-region.minx)*(region.maxy-region.miny)),faces=detectFacesFromSegments(segments,{minArea:1_000_000,maxArea:regionArea*.82,snapTol:14}).filter(f=>f.area<600_000_000),spaces=faces.map((f,i)=>({id:`spaceCandidate_${i+1}`,...f,confidence:clamp(.68+Math.min(.24,(f.wallIds?.length||0)*.03),.68,.92)})),doors=detectDoorCandidates(region,walls,arcWallSourceIds);
    return{lines,arcs,segmentedArcs,structural,pairs,walls,candidates:walls,spaces,doors,stairs,thicknessClusters,truncated:pairs.length>=18000,safetyExceeded:walls.length>=280};
  }
  ...
  function applyWallRecognition(){
    const p=state.wallRecognitionPreview,region=state.drawingRegions.find(r=>r.id===p?.regionId);if(!p||!region)return cancelWallRecognition();ensureFloorModel();const floor=activeFloor();if(floor){if(!floor.sourceRegionId)maybeAdoptFloorNameFromRegion(floor,region);floor.sourceRegionId=region.id;}if(p.safetyExceeded){alert(t('recognition.safetyExceeded'));return;}pushHistory();const wallMap=new Map(),created=[];
    ...
    if(createWalls){const baselines=recognitionBaselineWallSignatures(region.id);for(const c of p.walls){const matched=recognizedWallSourceMatch(c,region.id);if(matched){if(baselines.has(candidateSignature(matched))){refreshRecognizedWallFromCandidate(matched,c);created.push(matched.id);}wallMap.set(c.id,matched.id);continue;}if(wallCandidateDuplicatesExisting(c))continue;const w={id:uid('wall'),type:'wall',layerId:'walls',a:{...c.a},b:{...c.b},thickness:Math.round(c.thickness/5)*5,geometry:c.geometry==='arc'?'arc':'straight',attachments:{},recognizedFromCad:true,sourceRegionId:region.id,recognitionSourceIds:c.sourceIds||[],recognitionConfidence:c.confidence||null,floorId:state.activeFloorId};if(c.geometry==='arc'){w.center={...c.center};w.radius=c.radius;w.startAngle=c.startAngle;w.sweep=c.sweep;}state.objects.push(w);wallMap.set(c.id,w.id);created.push(w.id);}}
    if(createDoors){for(const c of p.doors||[]){const wallId=wallMap.get(c.wallCandidateId);if(!wallId)continue;state.objects.push({id:uid('door'),type:'door',layerId:'doors',wallId,t:c.t,width:c.width,doorType:c.doorType||'hingedSingle',hinge:c.hinge||'start',swing:c.swing||1,slideDirection:1,recognizedFromCad:true,sourceRegionId:region.id,recognitionConfidence:c.confidence||null,floorId:state.activeFloorId});}}
    if(createStairs){for(const c of p.stairs||[])state.objects.push({id:uid('stair'),type:'stair',layerId:'stairs',stairType:'straight',polygon:c.polygon.map(q=>({...q})),axis:{...c.axis},axisBounds:{...c.axisBounds},treadCount:c.treadCount,recognizedFromCad:true,sourceRegionId:region.id,recognitionConfidence:c.confidence||null,floorId:state.activeFloorId});}
  ...
  function runCommand(raw){const command=String(raw||'').trim().toUpperCase();if(!command)return;if(state.commandPending==='zoom'){if(command==='E'||command==='EXTENTS'){fitAll();setCommandStatus(t('command.zoomExtents'),'strong');state.commandPending=null;return;}setCommandStatus(t('command.unknown',{command}),'error');return;}
    ...
    else if(command==='E'||command==='ERASE'){if(selectionIds().size){deleteSelectedObjects();setCommandStatus('ERASE','strong');}else{setTool('delete','modify');setCommandStatus(t('command.erase'),'strong');}}
    ...
    else if(command==='WALL'){setTool('wall','architecture');if(plan||state.cadMapping)setCommandStatus('WALL','strong');}
    else if(command==='DOOR'){setTool('door','architecture');if(plan||state.cadMapping)setCommandStatus('DOOR','strong');}
    else if(command==='WINDOW'){setTool('window','architecture');if(plan||state.cadMapping)setCommandStatus('WINDOW','strong');}
  ...
  window.addEventListener('keydown',e=>{
    ...
    if(!isTyping()&&e.key==='Enter'&&state.activeTool==='line'&&state.drawStart){e.preventDefault();finishContinuousCommand();}
    ...
    if(e.key==='Escape'){if(!dom.confirmBackdrop.hidden){hideAppConfirm();return;}hideTooltip();hideContextMenu();closeFloorActionMenu();if(state.selectionDrag){state.selectionDrag=null;render();}else if(state.activeTool==='constraint'){cancelTransient();}else if(state.activeTool==='select'&&selectionIds().size){clearMultiSelection();renderPrimaryPanel();renderProperties();render();}else cancelTransient();}
  ...
  if(window.__PIENIPLAN_TEST_HOOK__){Object.assign(window.__PIENIPLAN_TEST_HOOK__,{state,detectWallCandidates,beginWallRecognition,applyWallRecognition,cancelWallRecognition,render,updateAll,fitAll,fitBounds,switchToolset,setTool,rebuildObjectSnapIndex,renderCadLayersPanel,saveProjectFile,makeProjectPayload,dxfDoorEntities,constrainTracking,applyTrimExtendAtPoint,ensureFloorModel,setActiveFloor,addFloor,renderPlanFloorPanel,getPlanObjects,detectClosedWallFaces,getLinkedCadRenderCache,runCommand,trimArchitecturalWallOverruns,cleanupArchitecturalWallTopology,suggestedFloorNameFromRegion,repairDefaultFloorNamesFromRegions,recognitionBaselineWallSignatures,recognizedWallSourceMatch});}

  dom.dialogBackdrop.hidden=true;dom.mappingBackdrop.hidden=true;dom.recognitionBackdrop.hidden=true;dom.confirmBackdrop.hidden=true;dom.commandBar.hidden=false;i18n.apply(document);state.inspectorSplit=safeReadInspectorSplit();state.theme=safeReadTheme();applyTheme(state.theme,{persist:false});installTooltips();updateEmptyState();renderToolRail();updateAll();if(dom.aboutVersion)dom.aboutVersion.textContent=`Version ${VERSION} · Build ${BUILD}`;setCommandStatus(t('command.ready'));updateContinueCard();history.replaceState({[ROUTE_MARKER]:true,view:'start',toolset:state.toolset},'',location.href);showStartScreen({historyMode:'none'});setTimeout(resizeCanvas,0);console.info(`PieniPlan v${VERSION} · Build ${BUILD}`);
})();

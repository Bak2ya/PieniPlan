(() => {
  ...
  function ensureSpaceMetadata(){
    ...
    for(const space of state.objects.filter(o=>o.type==='space'&&!o.floorId)){
      space.floorId=state.activeFloorId||state.floors[0]?.id||'floor_1';space.spaceUuid=space.spaceUuid||makeStableUuid();space.spaceType=space.spaceType||'unspecified';space.name=space.name||nextSpaceName(space.floorId);changed=true;
    }
    return changed;
  }
  ...
  function drawSpacePlan(obj){
    ...
    const c=toScreenCss(polygonCentroid(obj.polygon)),name=obj.name||spaceDefaultName(1),area=`${formatNumber(obj.areaM2||Math.abs(polygonArea(obj.polygon))/1e6,2)} m²`;
  ...
  function drawSpaceGapDiagnostic(){
    const d=state.spaceGapDiagnostic;if(!d||state.toolset!=='plan'||!d.candidates?.length)return;const styles=getComputedStyle(document.documentElement),danger=styles.getPropertyValue('--danger').trim()||'#F85149';ctx.save();ctx.strokeStyle=danger;ctx.fillStyle=danger;ctx.lineCap='round';
  ...
  function findSpaceBoundaryGapCandidates(p,{maxGap=900,limit=8}={}){
    const walls=getPlanObjects().filter(o=>o.type==='wall'),raw=[];for(const wall of walls)for(const endpoint of['a','b']){if(wall.attachments?.[endpoint])continue;const a=wall[endpoint];let best=null;for(const other of walls){if(other.id===wall.id)continue;const pr=wallProjectPoint(a,other),d=pr.distance;if(d<3||d>maxGap)continue;if(!best||d<best.distance)best={a:{...a},b:{...pr.point},distance:d,wallId:wall.id,otherWallId:other.id,endpoint,targetT:pr.t};}if(best){const mid={x:(best.a.x+best.b.x)/2,y:(best.a.y+best.b.y)/2};best.score=distance(mid,p)*.08+best.distance;raw.push(best);}}
    const unique=[];for(const g of raw.sort((a,b)=>a.score-b.score)){if(unique.some(u=>(u.wallId===g.otherWallId&&u.otherWallId===g.wallId)||distance(u.a,g.a)<3&&distance(u.b,g.b)<3))continue;unique.push(g);if(unique.length>=limit)break;}return unique;
  }
  ...
  function renderPlanFloorPanel(){ensureFloorModel();ensureSpaceMetadata();closeFloorActionMenu();dom.primaryControls.hidden=false;dom.primaryControls.innerHTML='';dom.primaryList.className='panel-list';dom.primaryList.innerHTML='';const toolbar=document.createElement('div');toolbar.className='floor-toolbar';const add=document.createElement('button');add.className='small-button';add.textContent=t('floor.add');add.addEventListener('click',addFloor);toolbar.append(add);dom.primaryControls.append(toolbar);const list=document.createElement('div');list.className='floor-list';for(const floor of state.floors){const group=document.createElement('div');group.className='floor-tree-group';const row=document.createElement('div');row.className=`floor-row ${floor.id===state.activeFloorId?'active':''}`;const objs=state.objects.filter(o=>o.floorId===floor.id&&(isSemanticObject(o)||o.type==='line')),spaces=objs.filter(o=>o.type==='space');row.innerHTML=`<div><div class="floor-name">${escapeHtml(floor.name)}</div><div class="floor-meta">${escapeHtml(t('floor.objectsAndSpaces',{count:objs.length,spaces:spaces.length}))}</div></div><button class="mini-action" aria-label="${escapeHtml(t('action.more'))}">•••</button>`;row.addEventListener('dblclick',e=>{if(!e.target.closest('button'))beginInlineFloorRename(floor,row);});row.addEventListener('click',e=>{if(!e.target.closest('button')&&!e.target.closest('input'))setActiveFloor(floor.id);});row.querySelector('button').addEventListener('click',e=>{e.stopPropagation();showFloorActionMenu(floor,e.currentTarget,row);});group.append(row);
      if(floor.id===state.activeFloorId&&spaces.length){const children=document.createElement('div');children.className='floor-space-list';for(const space of spaces){const srow=document.createElement('div');srow.className=`floor-space-row ${isSelectedId(space.id)?'selected':''}`;srow.dataset.spaceId=space.id;const typeLabel=t(`spaceType.${space.spaceType||'unspecified'}`);srow.innerHTML=`<span class="floor-space-branch" aria-hidden="true">└</span><div class="floor-space-main"><div class="floor-space-name">${escapeHtml(space.name||nextSpaceName(floor.id))}</div><div class="floor-space-meta">${escapeHtml(typeLabel)} · ${escapeHtml(formatNumber(space.areaM2||Math.abs(polygonArea(space.polygon||[]))/1e6,2))} m²</div></div>`;srow.addEventListener('click',()=>{selectOnly(space.id);state.selectedReferenceId=null;state.selectedRegionId=null;renderPlanFloorPanel();renderProperties();render();});srow.addEventListener('dblclick',e=>{e.preventDefault();e.stopPropagation();beginInlineSpaceRename(space,srow);});children.append(srow);}group.append(children);}list.append(group);}dom.primaryList.append(list);const floor=activeFloor();const ref=document.createElement('div');ref.className='floor-reference-section';const region=floor?.sourceRegionId?state.drawingRegions.find(r=>r.id===floor.sourceRegionId):null;ref.innerHTML=`<div class="section-heading">${escapeHtml(t('floor.reference'))}</div><div class="section-copy">${escapeHtml(region?region.name:t('floor.noReference'))}</div>`;const actions=document.createElement('div');actions.className='plan-reference-row';const re=document.createElement('button');re.className='small-button';re.textContent=t('floor.reRecognize');re.disabled=!region;re.addEventListener('click',()=>region&&beginWallRecognition(region));const cad=document.createElement('button');cad.className='small-button';cad.textContent=t('floor.editCad');cad.disabled=!state.sourceDxfName&&!state.objects.some(isCadObject);cad.addEventListener('click',()=>switchToolset('cad',{skipMapping:true}));actions.append(re,cad);ref.append(actions);dom.primaryList.append(ref);}

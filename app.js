(() => {
  ...
  const planToolCatalog = [
    ...
    { id: 'constraint', labelKey: 'tool.constraint', icon: 'vector', ready: true, tooltipKey: 'tooltip.constraintTool' },
    { separator: true },
  ...
  function objectOnActiveFloor(o){return !isSemanticObject(o)||!o.floorId||o.floorId===state.activeFloorId;}
  function semanticObjectsOnActiveFloor(){ensureFloorModel();return state.objects.filter(o=>isSemanticObject(o)&&objectOnActiveFloor(o));}
  ...
  function renameFloor(floor){const value=prompt(t('floor.renamePrompt'),floor.name);if(value==null||!value.trim())return;pushHistory();floor.name=value.trim();markDirty(true);updateAll();}
  function deleteFloor(floor){ensureFloorModel();if(state.floors.length<=1){alert(t('floor.keepOne'));return;}if(!confirm(t('floor.deleteConfirm',{name:floor.name})))return;pushHistory();state.objects=state.objects.filter(o=>!isSemanticObject(o)||o.floorId!==floor.id);state.floors=state.floors.filter(f=>f.id!==floor.id);state.activeFloorId=state.floors[0].id;clearMultiSelection();markDirty(true);rebuildObjectSnapIndex();updateAll();}
  ...
  function helpHtml(section){const ko=i18n.language==='ko';const data={
    ...
    plan:ko?`<h2>Plan Mode</h2><p>건물의 층별 평면을 구조화하는 모드입니다. 오른쪽 <strong>층</strong> 탭에서 현재 층을 바꾸고, <strong>팔레트</strong>에서 벽·문·창·공간·치수 도구를 바로 선택할 수 있습니다.</p><h3>CAD 구조 인식</h3><ul><li>여러 평행 제도선을 하나의 Wall Band로 병합</li><li>작은 끝점/코너 오차를 제한적으로 연결</li><li>문 개구부와 여닫이 호를 문 후보로 인식</li><li>반복 계단선을 하나의 계단 의미 객체로 축약</li><li>분절된 원호/곡선은 가능한 경우 하나의 의미 곡선으로 복원</li></ul><h3>편집</h3><p>클릭 선택과 드래그 다중 선택을 지원합니다. SNAP은 배치 보조이고 Constraint는 지속 관계입니다. 수평/수직/평행/직각/길이/각도/기준축 제약을 사용할 수 있습니다.</p><p><strong>재인식</strong>은 현재 층에 연결된 CAD 영역을 다시 분석합니다. 사용자 작업을 조용히 삭제하지 않도록 새 후보 검토 흐름을 사용합니다.</p>`:`<h2>Plan Mode</h2><p>Plan Mode structures a building as separate floors and semantic objects. Use the Floors tab to switch floors and Palette to select Plan tools.</p>`,
    ...
    files:ko?`<h2>파일과 프로젝트</h2><ul><li><strong>새 도면</strong>: 현재 작업을 초기화합니다.</li><li><strong>프로젝트 열기/저장</strong>: .pieniplan 파일에 층, Plan/CAD 객체, 레이어, 참조, 카메라와 인식 이력을 저장합니다.</li><li><strong>DXF 열기</strong>: CAD Mode에서 편집 가능한 DXF를 엽니다.</li><li><strong>DXF 내보내기</strong>: 현재 도면을 DXF로 생성합니다.</li></ul><p>원본 DXF는 자동으로 덮어쓰지 않습니다.</p>`:`<h2>Files & projects</h2><p>.pieniplan stores the editable project state. DXF export is separate and never silently overwrites the source DXF.</p>`,
    shortcuts:ko?`<h2>단축키</h2><table class="help-shortcuts"><tr><td><kbd>Ctrl/Cmd+Z</kbd></td><td>실행 취소</td></tr><tr><td><kbd>Ctrl/Cmd+Shift+Z</kbd></td><td>다시 실행</td></tr><tr><td><kbd>Ctrl/Cmd+S</kbd></td><td>프로젝트 저장</td></tr><tr><td><kbd>Delete</kbd></td><td>선택 객체 삭제</td></tr><tr><td><kbd>Esc</kbd></td><td>현재 작업 취소 / 선택 해제</td></tr><tr><td><kbd>F3</kbd></td><td>SNAP</td></tr><tr><td><kbd>F7</kbd></td><td>GRID</td></tr><tr><td><kbd>F8</kbd></td><td>ORTHO</td></tr><tr><td><kbd>F10</kbd></td><td>POLAR</td></tr><tr><td><kbd>L</kbd></td><td>LINE</td></tr><tr><td><kbd>TR</kbd> / <kbd>EX</kbd></td><td>TRIM / EXTEND</td></tr></table>`:`<h2>Shortcuts</h2><table class="help-shortcuts"><tr><td><kbd>Ctrl/Cmd+Z</kbd></td><td>Undo</td></tr><tr><td><kbd>Ctrl/Cmd+Shift+Z</kbd></td><td>Redo</td></tr><tr><td><kbd>Ctrl/Cmd+S</kbd></td><td>Save project</td></tr><tr><td><kbd>F3/F7/F8/F10</kbd></td><td>SNAP / GRID / ORTHO / POLAR</td></tr></table>`,
  ...
  function visibleSelectableObjects(){
    return state.objects.filter(o=>{
      if(state.toolset==='plan'&&(isCadObject(o)||!objectOnActiveFloor(o)))return false;
  ...
  function drawPlanWorkspace() {
    ...
    if(layerVisible('spaces')) for(const obj of state.objects) if(obj.type==='space'&&objectOnActiveFloor(obj)) drawSpacePlan(obj);
    if(layerVisible('stairs')) for(const obj of state.objects) if(obj.type==='stair'&&objectOnActiveFloor(obj)) drawStairPlan(obj);
    for (const obj of state.objects) {
      if (obj.type === 'wall' && objectOnActiveFloor(obj) && layerVisible('walls')) drawWallPlan(obj, isSelectedId(obj.id) ? sel : wall);
    }
    for (const obj of state.objects) {
      if (obj.type === 'door' && objectOnActiveFloor(obj) && layerVisible('doors')) drawOpeningPlan(obj, 'door', isSelectedId(obj.id) ? sel : styles.getPropertyValue('--door').trim());
      else if (obj.type === 'window' && objectOnActiveFloor(obj) && layerVisible('windows')) drawOpeningPlan(obj, 'window', isSelectedId(obj.id) ? sel : styles.getPropertyValue('--window').trim());
      else if (obj.type === 'dimension' && objectOnActiveFloor(obj) && layerVisible('dimensions')) drawDimension(obj, isSelectedId(obj.id) ? sel : dim);
  ...
  function syncDependentsOfWall(parentId,visited=new Set()){if(visited.has(parentId))return;visited.add(parentId);const parent=state.objects.find(o=>o.id===parentId&&o.type==='wall');if(!parent)return;for(const wall of state.objects){if(wall.type!=='wall'||wall.id===parentId)continue;let changed=false;for(const endpoint of['a','b']){const att=wall.attachments?.[endpoint];if(att?.wallId===parentId){wall[endpoint]=wallPointAt(parent,att.t);changed=true;}}const c=ensureWallConstraints(wall);if(c.reference?.wallId===parentId){enforceWallConstraints(wall);changed=true;}if(changed)syncDependentsOfWall(wall.id,visited);}refreshSpaces();}
  ...
  dom.fileOpenProjectAction?.addEventListener('click',()=>{closeTopMenus();dom.projectFileInput.click();});

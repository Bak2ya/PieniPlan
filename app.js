(() => {
  ...
  function helpHtml(section){const ko=i18n.language==='ko';const data={
    ...
    files:ko?`<h2>파일과 프로젝트</h2><ul><li><strong>새 도면</strong>: 현재 작업을 초기화합니다.</li><li><strong>프로젝트 열기/저장</strong>: .pieniplan 작업 상태에 층, Plan/CAD 객체, 레이어, 참조, 카메라와 인식 이력을 저장합니다. 직접 파일 쓰기를 지원하는 브라우저에서는 처음 선택한 프로젝트 파일을 이후 저장에서 갱신합니다. 지원하지 않는 브라우저에서는 다운로드하지 않고 이 브라우저의 로컬 프로젝트 저장소에 저장합니다.</li><li><strong>프로젝트 파일 다운로드</strong>: 다른 기기나 브라우저로 옮길 수 있는 .pieniplan 파일이 필요할 때 명시적으로 다운로드합니다.</li><li><strong>DXF 열기</strong>: CAD Mode에서 편집 가능한 DXF를 엽니다.</li><li><strong>DXF 내보내기</strong>: 현재 도면을 DXF로 생성합니다.</li></ul><p>원본 DXF는 자동으로 덮어쓰지 않습니다.</p>`:`<h2>Files & projects</h2><p>.pieniplan stores the editable project state. Browsers with direct file access update the chosen project file in place. Other browsers save the working project in browser-local storage without silently downloading a file. Use Download Project File when you explicitly need a portable .pieniplan file. DXF export is separate and never silently overwrites the source DXF.</p>`,
  ...
  function wallCandidateDuplicatesExisting(c,regionId=null,floorId=null){
  ...
  function applyWallRecognition(){
    const p=state.wallRecognitionPreview,region=state.drawingRegions.find(r=>r.id===p?.regionId);if(!p||!region)return cancelWallRecognition();if(p.safetyExceeded){alert(t('recognition.safetyExceeded'));return;}pushHistory();const floor=ensureFloorForRegion(region);state.activeFloorId=floor.id;const wallMap=new Map(),created=[];
    ...
    if(createWalls){const baselines=recognitionBaselineWallSignatures(region.id);for(const c of p.walls){const matched=recognizedWallSourceMatch(c,region.id);if(matched){matched.floorId=floor.id;if(baselines.has(candidateSignature(matched))){refreshRecognizedWallFromCandidate(matched,c);created.push(matched.id);}wallMap.set(c.id,matched.id);continue;}if(wallCandidateDuplicatesExisting(c,region.id,floor.id))continue;const w={id:uid('wall'),type:'wall',layerId:'walls',a:{...c.a},b:{...c.b},thickness:Math.round(c.thickness/5)*5,geometry:c.geometry==='arc'?'arc':'straight',attachments:{},recognizedFromCad:true,sourceRegionId:region.id,recognitionSourceIds:c.sourceIds||[],recognitionConfidence:c.confidence||null,floorId:floor.id};if(c.geometry==='arc'){w.center={...c.center};w.radius=c.radius;w.startAngle=c.startAngle;w.sweep=c.sweep;}state.objects.push(w);wallMap.set(c.id,w.id);created.push(w.id);}}
    if(createDoors){for(const c of p.doors||[]){const wallId=wallMap.get(c.wallCandidateId);if(!wallId)continue;state.objects.push({id:uid('door'),type:'door',layerId:'doors',wallId,t:c.t,width:c.width,doorType:c.doorType||'hingedSingle',hinge:c.hinge||'start',swing:c.swing||1,slideDirection:1,recognizedFromCad:true,sourceRegionId:region.id,recognitionSourceIds:[...(c.sourceIds||[])],recognitionConfidence:c.confidence||null,floorId:floor.id});}}
    if(createStairs){for(const c of p.stairs||[])state.objects.push({id:uid('stair'),type:'stair',layerId:'stairs',stairType:'straight',polygon:c.polygon.map(q=>({...q})),axis:{...c.axis},axisBounds:{...c.axisBounds},treadCount:c.treadCount,recognizedFromCad:true,sourceRegionId:region.id,recognitionConfidence:c.confidence||null,floorId:floor.id});}
  ...
  function runCommand(raw){const command=String(raw||'').trim().toUpperCase();if(!command){if(state.commandPending){setCommandStatus(t('command.finished'),'strong');state.commandPending=null;return;}if(state.lastCommand){return runCommand(state.lastCommand);}setCommandStatus(t('command.noPrevious'),'strong');dom.commandInput.focus();return;}
    ...
    else if(command==='DI'||command==='DIST'){state.lastCommand='DI';setTool('measure','dimension');setCommandStatus(t('command.distance'),'strong');}

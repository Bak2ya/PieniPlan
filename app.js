(() => {
  ...
  const doorTypeCatalog = [
    ...
    { id:'doubleActingSingle', labelKey:'doorType.doubleActingSingle' },
    { id:'doubleActingDouble', labelKey:'doorType.doubleActingDouble' },
    { id:'fireDoor', labelKey:'doorType.fireDoor' },
    { id:'fireShutter', labelKey:'doorType.fireShutter' },
  ...
  const state = {
    ...
    cadRenderRevision: 0,
    floorMenuEl: null,
    expandedFloorIds: new Set(['floor_1']),
    confirmAction: null,
  ...
  function floorForRegion(regionId){return state.floors.find(f=>f.sourceRegionId===regionId)||null;}
  ...
  function cadSelectableObjects(){const out=cadWorkObjects().filter(o=>cadLayerVisible(o.cadLayer||'0'));if(state.cadPlanOverlay)out.push(...cadPlanOverlayObjects().filter(o=>o.type==='space'||cadLayerVisible(cadLayerForObject(o))));return out;}
  function setCadWorkRegion(regionId,{fit=false}={}){const region=regionId?state.drawingRegions.find(r=>r.id===regionId):null;state.cadWorkRegionId=region?.id||null;state.selectedRegionId=region?.id||null;clearMultiSelection();state.hoveredObjectId=null;state.trimPreview=null;if(fit&&region)fitBounds(region);renderPrimaryPanel();renderProperties();render();}
  function semanticObjectsOnActiveFloor(){ensureFloorModel();return getPlanObjects().filter(isSemanticObject);}
  function addFloor(){ensureFloorModel();pushHistory();let n=1,name;const names=new Set(state.floors.map(f=>f.name));do{name=`${n++}F`;}while(names.has(name));const floor={id:`floor_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,name,sourceRegionId:null};state.floors.push(floor);state.activeFloorId=floor.id;state.expandedFloorIds.add(floor.id);clearMultiSelection();markDirty(true);updateAll();switchInspector('primary');}
  ...
  function helpHtml(section){const ko=i18n.language==='ko';const data={
    ...
    files:ko?`<h2>파일과 프로젝트</h2><ul><li><strong>새 도면</strong>: 현재 작업을 초기화합니다.</li><li><strong>프로젝트 열기/저장</strong>: .ppln 작업 상태에 층, Plan/CAD 객체, 공간 이름/유형, 레이어, 참조, 카메라와 인식 이력을 저장합니다. 기존 .pieniplan 파일도 계속 열 수 있습니다. 쓰기 권한을 가진 파일 핸들로 연 프로젝트는 이후 저장에서 그 파일을 갱신합니다. 그 외에는 다운로드하지 않고 이 브라우저의 로컬 프로젝트 저장소에 저장합니다.</li><li><strong>프로젝트 파일 다운로드</strong>: 다른 기기나 브라우저로 옮길 수 있는 .ppln 파일이 필요할 때 명시적으로 다운로드합니다.</li><li><strong>DXF 열기</strong>: CAD Mode에서 편집 가능한 DXF를 엽니다.</li><li><strong>DXF 내보내기</strong>: 현재 도면을 DXF로 생성합니다. 지원 브라우저에서는 운영체제의 저장 창에서 위치와 파일명을 직접 정하며, 지원하지 않는 브라우저에서는 파일명을 먼저 확인한 뒤 브라우저 다운로드 위치를 사용한다고 안내합니다.</li></ul><p>원본 DXF는 자동으로 덮어쓰지 않습니다.</p>`:`<h2>Files & projects</h2><p>.ppln stores the editable project state; legacy .pieniplan files remain readable. Projects opened with a writable file handle update that file in place. Otherwise Save uses browser-local storage without silently downloading a file. Use Download Project File when you explicitly need a portable .ppln file. DXF export is separate and never silently overwrites the source DXF. When the browser supports a native Save As picker, you choose the location and file name; otherwise PieniPlan asks for the file name and clearly explains that the browser controls the download location.</p>`,
    shortcuts:ko?`<h2>단축키</h2><table class="help-shortcuts"><tr><td><kbd>Ctrl/Cmd+Z</kbd></td><td>실행 취소</td></tr><tr><td><kbd>Ctrl/Cmd+Shift+Z</kbd></td><td>다시 실행</td></tr><tr><td><kbd>Ctrl/Cmd+S</kbd></td><td>프로젝트 저장</td></tr><tr><td><kbd>Delete</kbd></td><td>선택 객체 삭제</td></tr><tr><td><kbd>Esc</kbd></td><td>현재 작업 취소 / 선택 해제</td></tr><tr><td><kbd>F3</kbd></td><td>SNAP</td></tr><tr><td><kbd>F7</kbd></td><td>GRID</td></tr><tr><td><kbd>F8</kbd></td><td>ORTHO</td></tr><tr><td><kbd>F10</kbd></td><td>POLAR</td></tr><tr><td><kbd>Shift</kbd></td><td>Plan: GLOBAL+REF 45° 기하 제약 / 선 이동축 제한 · CAD: ORTHO 임시 반전</td></tr><tr><td><kbd>Ctrl</kbd></td><td>Plan: 누르는 동안 뒤쪽 CAD/참조 도면 스냅 허용</td></tr><tr><td><kbd>L</kbd></td><td>LINE</td></tr><tr><td><kbd>TR</kbd> / <kbd>EX</kbd></td><td>TRIM / EXTEND</td></tr><tr><td><kbd>Enter</kbd></td><td>빈 커맨드에서 마지막 명령 반복 / 진행 중 단계 확정</td></tr><tr><td><kbd>S</kbd></td><td>STRETCH 예약(현재 미지원) · 저장은 Ctrl/Cmd+S</td></tr></table>`:`<h2>Shortcuts</h2><table class="help-shortcuts"><tr><td><kbd>Ctrl/Cmd+Z</kbd></td><td>Undo</td></tr><tr><td><kbd>Ctrl/Cmd+Shift+Z</kbd></td><td>Redo</td></tr><tr><td><kbd>Ctrl/Cmd+S</kbd></td><td>Save project</td></tr><tr><td><kbd>F3/F7/F8/F10</kbd></td><td>SNAP / GRID / ORTHO / POLAR</td></tr><tr><td><kbd>Shift</kbd></td><td>Plan: GLOBAL+REF 45° geometry constraints / constrained line movement · CAD: temporary ORTHO inversion</td></tr><tr><td><kbd>Ctrl</kbd></td><td>Plan: temporarily include CAD/reference geometry in SNAP</td></tr><tr><td><kbd>Enter</kbd></td><td>Repeat the last command when idle / confirm the current command step</td></tr><tr><td><kbd>S</kbd></td><td>Reserved for STRETCH (not implemented yet); save with Ctrl/Cmd+S</td></tr></table>`,
  ...
  function updateContinueCard() {
    ...
    if (!show) return;
    if(local){dom.continueWorkDetail.textContent=t('start.continueLocalDetail',{name:local.name||'PieniPlan.ppln'});return;}
  ...
  function referencesForRender(){
    ...
    const regionId=activeFloor()?.sourceRegionId||null;
    return state.references.filter(ref=>ref.visible&&(ref.type!=='linkedCadRegion'||ref.regionId===regionId));
  }
  ...
  function drawPlanWorkspace() {
    ...
    if(!linkedCadReference) for (const obj of cadObjectsForRegion(region)) {if (!isCadObject(obj)) continue;if (!cadLayerVisible(obj.cadLayer)) continue;const selected=isSelectedId(obj.id),preview=isSelectionPreviewId(obj.id);drawCadObject(obj, selected ? sel : preview ? hover : muted, selected ? 2 : preview ? 1.8 : 1);}
  ...
  function drawSpaceGapDiagnostic(){
  ...
  function drawCadWorkspace() {
    ...
    for(const obj of cadWorkObjects()){if(!cadLayerVisible(obj.cadLayer||'0'))continue;const selected=isSelectedId(obj.id),preview=isSelectionPreviewId(obj.id),hovered=obj.id===state.hoveredObjectId&&!selected&&!preview;drawCadObject(obj,selected?sel:preview?hover:hovered?hover:line,selected?2:preview?1.8:hovered?1.8:1.25);}
  ...
  function drawOpeningPlan(obj, kind, color) {
    ...
    if(kind==='door'){
      ...
      else if(type==='hingedDouble'){drawHingedLeaf(obj,g,stroke,0,.5,{hinge:'start'});drawHingedLeaf(obj,g,stroke,.5,1,{hinge:'end'});}
      ...
      else if(type==='doubleActingDouble'){for(const side of[doorSwingSide(obj),-doorSwingSide(obj)]){drawHingedLeaf(obj,g,stroke,0,.5,{hinge:'start',swingSide:side,alpha:side===doorSwingSide(obj)?1:.42});drawHingedLeaf(obj,g,stroke,.5,1,{hinge:'end',swingSide:side,alpha:side===doorSwingSide(obj)?1:.42});}}
  ...
  function drawTrimPreview(){
    const p=state.trimPreview;if(!p||state.activeTool!=='trim')return;
  ...
  function updateContextBar(){
    const contextTools=new Set(['line','wall','door','window','constraint','space']);const shouldShow=Boolean(state.calibration)||contextTools.has(state.activeTool);
  ...
  function visibleCadLines(excludeId=null){return cadWorkObjects().filter(o=>o.type==='cadLine'&&o.id!==excludeId&&cadLayerVisible(o.cadLayer||'0'));}
  function computeCadTrimSegment(target,click){
    if(!target||target.type!=='cadLine'||!cadObjectInWorkScope(target))return null;const hits=[];for(const other of visibleCadLines(target.id)){const x=segmentIntersection(target.a,target.b,other.a,other.b);if(x&&x.t>1e-6&&x.t<1-1e-6)hits.push(x.t);}if(!hits.length)return null;
    const pr=projectPointToSegment(click,target.a,target.b),cuts=[0,...[...new Set(hits.map(v=>Math.round(v*1e7)/1e7))].sort((a,b)=>a-b),1];let k=0;for(let i=0;i<cuts.length-1;i++)if(pr.t>=cuts[i]-1e-8&&pr.t<=cuts[i+1]+1e-8){k=i;break;}const t0=cuts[k],t1=cuts[k+1],point=t=>({x:target.a.x+(target.b.x-target.a.x)*t,y:target.a.y+(target.b.y-target.a.y)*t});return{targetId:target.id,t0,t1,a:point(t0),b:point(t1),cuts:[...(t0>1e-8?[point(t0)]:[]),...(t1<1-1e-8?[point(t1)]:[])]};
  }
  function updateTrimPreview(raw){
    state.trimPreview=null;if(state.activeTool!=='trim')return;const obj=hitObject(raw);if(state.toolset==='plan'){if(!obj||(obj.type!=='line'&&!(obj.type==='wall'&&!isArcWall(obj))))return;state.trimPreview=computePlanTrimSegment(obj,raw);return;}if(obj?.type==='cadLine')state.trimPreview=computeCadTrimSegment(obj,raw);
  }
  function trimCadLineAtClick(target,click){
    const preview=computeCadTrimSegment(target,click);if(!preview)return false;const {t0,t1}=preview,point=t=>({x:target.a.x+(target.b.x-target.a.x)*t,y:target.a.y+(target.b.y-target.a.y)*t});pushHistory();
  ...
  function attachEndpointToExactBoundary(target,endpoint,boundary,point){
    if(target?.type!=='wall'||boundary?.type!=='wall')return false;target.attachments=target.attachments||{};const pr=wallProjectPoint(point,boundary),endTol=.003;
    if(pr.t<=endTol){target[endpoint]={...boundary.a};target.attachments[endpoint]={wallId:boundary.id,t:0,kind:'coincident',targetEndpoint:'a',autoJunction:false};}
    else if(pr.t>=1-endTol){target[endpoint]={...boundary.b};target.attachments[endpoint]={wallId:boundary.id,t:1,kind:'coincident',targetEndpoint:'b',autoJunction:false};}
  ...
  function extendPlanLinearAtClick(target,click){
    const ends=linearEnds(target);if(!ends)return false;const da=distance(click,ends[0]),db=distance(click,ends[1]),extendA=da<=db,candidates=[];
    for(const other of planLinearObjects(target.id)){const oe=linearEnds(other);if(!oe)continue;const x=infiniteLineSegmentIntersection(ends[0],ends[1],oe[0],oe[1]);if(!x)continue;if(extendA&&x.t<-.00001)candidates.push({...x,boundary:other});if(!extendA&&x.t>1.00001)candidates.push({...x,boundary:other});}
    if(!candidates.length)return false;candidates.sort((x,y)=>extendA?y.t-x.t:x.t-y.t);const hit=candidates[0],endpoint=extendA?'a':'b',before={...target[endpoint]};pushHistory();
    if(target.type==='wall'){target.attachments=target.attachments||{};delete target.attachments[endpoint];detachAutoDependentsAtEndpoint(target.id,endpoint);}
  ...
  function applyTrimExtendAtPoint(raw,mode){const obj=hitObject(raw);if(!obj)return false;const actual=state.toolset==='plan'?mode:(state.shiftDown?(mode==='trim'?'extend':'trim'):mode);let ok=false;if(state.toolset==='cad'&&obj.type==='cadLine')ok=actual==='trim'?trimCadLineAtClick(obj,raw):extendCadLineAtClick(obj,raw);else if(state.toolset==='plan'&&(obj.type==='line'||(obj.type==='wall'&&!isArcWall(obj))))ok=actual==='trim'?trimPlanLinearAtClick(obj,raw):extendPlanLinearAtClick(obj,raw);if(ok)recordEdit(actual,{targetId:obj.id,toolset:state.toolset});setCommandStatus(ok?t(actual==='trim'?'command.trimApplied':'command.extendApplied'):t(actual==='trim'?'command.trimNoBoundary':'command.extendNoBoundary'),ok?'strong':'error');return ok;}
  ...
  function switchToolset(next,{skipMapping=false,historyMode='push',regionId=undefined}={}){
    ...
    if(next==='cad'){if(regionId!==undefined)state.cadWorkRegionId=state.drawingRegions.some(r=>r.id===regionId)?regionId:null;else if(previous==='plan'){const rid=activeFloor()?.sourceRegionId;state.cadWorkRegionId=state.drawingRegions.some(r=>r.id===rid)?rid:null;}state.cadPlanOverlay=false;}
  ...
  function detectClosedWallFaces(){
    ...
    const walls=getPlanObjects().filter(o=>o.type==='wall');if(walls.length<3){state.spaceFaceCache={revision,floorId,faces:[]};return[];}
    ...
    const faces=detectFacesFromSegments(segments,{minArea:10000,ignoreInteriorCrossings:false});state.spaceFaceCache={revision,floorId,faces};return faces;
  }
  ...
  function nearestSnap(p,excludeObjectId=null){
    ...
    if(state.objectSnapIndex)queryObjectSnapIndex(p,threshold,q=>{const o=q.objectId&&state.objects.find(x=>x.id===q.objectId);if(state.toolset==='plan'&&o&&(!objectOnActiveFloor(o)||isCadObject(o)||(o.type==='line'&&!o.floorId)))return;if(state.toolset==='cad'&&o&&(!cadObjectInWorkScope(o)||!cadLayerVisible(o.cadLayer||'0')))return;test(q,q.kind,state.toolset==='cad'?'cad':'plan');});else for(const o of state.objects){if(state.toolset==='plan'&&(!objectOnActiveFloor(o)||isCadObject(o)||(o.type==='line'&&!o.floorId)))continue;if(state.toolset==='cad'&&(!cadObjectInWorkScope(o)||!cadLayerVisible(o.cadLayer||'0')))continue;if(o.a)test({...o.a,objectId:o.id},'endpoint',state.toolset==='cad'?'cad':'plan');if(o.b)test({...o.b,objectId:o.id},'endpoint',state.toolset==='cad'?'cad':'plan');}
  ...
  function connectedWallAngles(wall){const out=[];if(!wall||wall.type!=='wall')return out;for(const ep of['a','b']){const att=wall.attachments?.[ep],parent=att&&state.objects.find(o=>o.id===att.wallId&&o.type==='wall');if(parent&&!isArcWall(parent))out.push(angleDeg(parent.a,parent.b));}for(const child of getPlanObjects().filter(o=>o.type==='wall'&&o.id!==wall.id&&!isArcWall(o))){for(const ep of['a','b'])if(child.attachments?.[ep]?.wallId===wall.id)out.push(angleDeg(child.a,child.b));}return [...new Set(out.map(normalize180))];}
  function constrainTracking(start,p){
    ...
    if(state.toolset==='plan'&&state.activeTool==='line'&&state.shiftDown){const candidates=[...angleFamily(0,45,'GLOBAL')];if(Number.isFinite(state.drawReferenceAngle))candidates.push(...angleFamily(state.drawReferenceAngle,45,'REF'));if(Math.abs(base)>1e-7)candidates.push(...angleFamily(base,45,'AXIS'));const picked=nearestAngleCandidate(start,p,candidates);state.shiftGuide=picked.angle==null?null:{angle:picked.angle,source:picked.source};return picked.point;}
  ...
  function constrainEndpointWithShift(src,endpoint,p){
    if(!src?.a||!src?.b||!['a','b'].includes(endpoint))return p;const anchor=endpoint==='a'?src.b:src.a,current=angleDeg(anchor,endpoint==='a'?src.a:src.b),candidates=[{angle:current,source:'CURRENT'},...angleFamily(0,45,'GLOBAL')];for(const a of connectedWallAngles(src))candidates.push(...angleFamily(a,45,'REF'));const base=baseAxisAngle();if(Math.abs(base)>1e-7)candidates.push(...angleFamily(base,45,'AXIS'));const picked=nearestAngleCandidate(anchor,p,candidates);state.shiftGuide=picked.angle==null?null:{angle:picked.angle,source:picked.source};return picked.point;
  }
  function constrainEndpointToOriginalAngle(src,endpoint,p){return constrainEndpointWithShift(src,endpoint,p);}
  function constrainBodyMoveDelta(src,delta){if(!state.shiftDown||state.toolset!=='plan'||!src?.a||!src?.b)return delta;const len=Math.hypot(delta.x,delta.y);if(len<1e-9)return delta;const candidates=[{angle:0,source:'GLOBAL'},{angle:90,source:'GLOBAL'},{angle:180,source:'GLOBAL'},{angle:270,source:'GLOBAL'}];for(const a of connectedWallAngles(src)){candidates.push({angle:a,source:'REF'},{angle:a+90,source:'REF'},{angle:a+180,source:'REF'},{angle:a+270,source:'REF'});}const picked=nearestAngleCandidate({x:0,y:0},{x:delta.x,y:delta.y},candidates);state.shiftGuide=picked.angle==null?null:{angle:picked.angle,source:picked.source};return picked.point;}
  ...
  function applyObjectDrag(p){const d=state.dragEdit;if(!d)return;const obj=state.objects.find(o=>o.id===d.objectId);if(!obj)return;const src=d.snapshot,rawDelta={x:p.x-d.start.x,y:p.y-d.start.y},delta=(state.toolset==='plan'&&d.mode==='body'&&(obj.type==='wall'||obj.type==='line'))?constrainBodyMoveDelta(src,rawDelta):rawDelta;ensureDragHistory();
  ...
  function onPointerMove(e){if(state.toolset==='plan'){state.ctrlDown=Boolean(e.ctrlKey);state.shiftDown=Boolean(e.shiftKey);}const s=fromPointerEvent(e);let p=screenCssToWorld(s);state.cursorWorld=p;dom.statusX.textContent=`X ${formatNumber(p.x,1)}`;dom.statusY.textContent=`Y ${formatNumber(p.y,1)}`;
    ...
    if(state.activeTool==='trim')updateTrimPreview(p);else state.trimPreview=null;
  ...
  function onPointerDown(e){host.focus();hideContextMenu();if(state.toolset==='plan'){state.ctrlDown=Boolean(e.ctrlKey);state.shiftDown=Boolean(e.shiftKey);}const s=fromPointerEvent(e);if(isCompactViewer()){if(e.button===0||e.pointerType==='touch'){e.preventDefault();beginViewerPointer(e,s);}return;}const panGesture=e.button===1||(e.button===0&&state.spaceDown);if(panGesture){e.preventDefault();if(state.spaceGesture)state.spaceGesture.used=true;state.pan={pointerId:e.pointerId,startScreen:s,startCamera:{...state.camera}};host.dataset.pan='true';canvas.setPointerCapture?.(e.pointerId);return;}const ctrlSnapClick=state.toolset==='plan'&&e.ctrlKey&&['line','wall','measure','move','copy','trim','extend'].includes(state.activeTool);if(e.button!==0&&!(ctrlSnapClick&&e.button===2))return;if(ctrlSnapClick)e.preventDefault();const editableLabel=hitEditableLabel(s);if(editableLabel)return;let raw=screenCssToWorld(s),p=nearestSnap(raw);
    ...
    if(state.activeTool==='trim'||state.activeTool==='extend'){const mode=state.activeCommand==='TR'?'trim':state.activeCommand==='EX'?'extend':state.activeTool;applyTrimExtendAtPoint(raw,mode);state.trimPreview=null;render();return;}
    if(state.activeTool==='line'||state.activeTool==='wall'){
  ...
  function projectBaseName(){const raw=state.sourceDxfName||state.projectFileName||'PieniPlan';return sanitizeFilename(String(raw).replace(/\.(dxf|ppln|pieniplan)$/i,''));}
  function projectFilePickerTypes(){return[{description:'PieniPlan Project',accept:{'application/json':['.ppln','.pieniplan']}}];}
  ...
  async function saveProjectFile({saveAs=false}={}){
    try{
      const payload=makeProjectPayload(),json=JSON.stringify(payload,null,2),name=`${projectBaseName()}.ppln`;
      // Save is deliberately non-download. If a project was opened through a writable
      // File System Access handle, update that exact file. Otherwise use browser-local
      // IndexedDB. Portable files are created only by the explicit export/download action.
      if(!saveAs&&state.projectFileHandle?.createWritable){await writeProjectToHandle(state.projectFileHandle,json,name);return;}
      await saveProjectToBrowserStore(json,(state.projectFileName||name).replace(/\.pieniplan$/i,'.ppln'));
    }catch(err){if(err?.name==='AbortError')return;console.error(err);alert(t('project.saveFailed',{message:String(err?.message||err)}));}
  }
  function downloadProjectFile(){try{const payload=makeProjectPayload(),json=JSON.stringify(payload,null,2),name=`${projectBaseName()}.ppln`;downloadBlob(new Blob([json],{type:'application/json;charset=utf-8'}),name);setCommandStatus(t('project.downloaded',{name}),'strong');}catch(err){console.error(err);alert(t('project.saveFailed',{message:String(err?.message||err)}));}}
  ...
  async function openProjectFile(file,{handle=null,localKey=null,skipConfirm=false,restoredLocal=false}={}){
    ...
    try{
      ...
      const d=raw.drawing,refs=[];state.editLog=Array.isArray(raw.debug?.editLog)?raw.debug.editLog.slice(-100):[];for(const rr of d.references||[]){const ref=await hydrateReference(rr);if(ref)refs.push(ref);}
      ...
      state.selectedObjectId=null;state.selectedObjectIds.clear();state.selectedReferenceId=null;state.selectedRegionId=null;state.wallRecognitionPreview=null;state.spaceGapDiagnostic=null;state.expandedFloorIds=new Set([state.activeFloorId]);state.history=[];state.future=[];state.layerFilter='';state.layerRevealRequested=false;state.dirty=Boolean(floorIsolationChanged||legacyEdgeChanged||spaceMetadataChanged||doorMetadataChanged||junctionRepairChanged);rebuildObjectSnapIndex();refreshSpaces();switchToolset(d.toolset==='cad'?'cad':'plan',{skipMapping:true});updateAll();setCommandStatus(restoredLocal?t('project.restoredLocal',{name:file.name}):t('project.opened',{name:file.name}),'strong');
  ...
  function renderCadLayersPanel(){
    ...
    const scope=document.createElement('div');scope.className='cad-scope-bar';const activeRegion=activeCadWorkRegion();const scopeCopy=document.createElement('div');scopeCopy.className='cad-scope-copy';scopeCopy.innerHTML=`<span>${escapeHtml(t('cadScope.label'))}</span><strong>${escapeHtml(activeRegion?.name||t('cadScope.full'))}</strong>`;const scopeActions=document.createElement('div');scopeActions.className='cad-scope-actions';const full=document.createElement('button');full.className='mini-action';full.textContent=t('cadScope.full');full.disabled=!activeRegion;full.addEventListener('click',()=>{setCadWorkRegion(null);fitAll();});const overlay=document.createElement('button');overlay.className=`mini-action ${state.cadPlanOverlay?'active':''}`;overlay.textContent=t('cadScope.planOverlay');overlay.setAttribute('aria-pressed',String(Boolean(state.cadPlanOverlay)));overlay.addEventListener('click',()=>{state.cadPlanOverlay=!state.cadPlanOverlay;clearMultiSelection();renderCadLayersPanel();render();});scopeActions.append(full,overlay);scope.append(scopeCopy,scopeActions);dom.primaryControls.appendChild(scope);
    ...
    for(const region of state.drawingRegions){
      ...
      main.innerHTML=`<strong title="${escapeHtml(region.name)}">${escapeHtml(region.name)}</strong><span>${escapeHtml(t('region.entities',{count:regionObjectCount(region).toLocaleString()}))}${state.cadWorkRegionId===region.id?` · ${escapeHtml(t('cadScope.active'))}`:''}</span>`;
  ...
  function resetProject(){if(state.dirty&&!confirm(t('confirm.newDrawing')))return;state.references=[];state.objects=[];state.drawingRegions=[];state.selectedRegionId=null;state.wallRecognitionPreview=null;state.spaceGapDiagnostic=null;state.expandedFloorIds=new Set(['floor_1']);state.selectedReferenceId=null;state.selectedObjectId=null;state.selectedObjectIds.clear();state.selectionDrag=null;state.hoveredObjectId=null;state.layerFilter='';state.baseAxisAngle=0;state.baseAxisWallId=null;state.history=[];state.future=[];state.editLog=[];state.activeCommand=null;state.spaceHoverPreview=null;state.cadWorkRegionId=null;state.cadPlanOverlay=false;state.nextId=1;state.camera={cx:0,cy:0,zoom:.12};state.cadMapping=null;state.cadLayerVisibility=new Map([['0',true]]);state.activeCadLayer='0';state.sourceDxfName=null;state.sourceDxfFingerprint=null;state.sourceDxfSize=0;state.sourceDxfLastModified=0;state.sourceDxfMainBounds=null;state.sourceDxfFullBounds=null;state.sourceDxfOutlierCount=0;state.projectFileName=null;state.projectFileHandle=null;state.projectLocalKey=null;state.recognitionHistory=[];state.floors=[{id:'floor_1',name:'1F',sourceRegionId:null}];state.activeFloorId='floor_1';state.dirty=false;rebuildObjectSnapIndex();setTool('select',state.toolset==='plan'?'plan':'select');updateAll();}

  function createSample(){if(state.dirty&&!confirm(t('confirm.newDrawing')))return;state.references=[];state.objects=[];state.drawingRegions=[];state.selectedRegionId=null;state.wallRecognitionPreview=null;state.selectedObjectIds.clear();state.selectionDrag=null;state.layerFilter='';state.baseAxisAngle=0;state.baseAxisWallId=null;state.history=[];state.future=[];state.editLog=[];state.activeCommand=null;state.spaceHoverPreview=null;state.cadWorkRegionId=null;state.cadPlanOverlay=false;state.nextId=1;const wall=(a,b,th=150)=>{const o={id:uid('wall'),type:'wall',layerId:'walls',a,b,thickness:th,geometry:'straight',attachments:{}};state.objects.push(o);return o;};const outer=[wall({x:0,y:0},{x:8000,y:0},180),wall({x:8000,y:0},{x:8000,y:6000},180),wall({x:8000,y:6000},{x:0,y:6000},180),wall({x:0,y:6000},{x:0,y:0},180)];const innerV=wall({x:4200,y:0},{x:4200,y:6000},150),innerH=wall({x:0,y:3100},{x:4200,y:3100},150);attachWallEndpoint(innerV,'a');attachWallEndpoint(innerV,'b');attachWallEndpoint(innerH,'a');attachWallEndpoint(innerH,'b');state.objects.push({id:uid('door'),type:'door',layerId:'doors',wallId:innerV.id,t:.48,width:900,doorType:'hingedSingle',hinge:'start',swing:1,slideDirection:1},{id:uid('window'),type:'window',layerId:'windows',wallId:outer[2].id,t:.7,width:1600},{id:uid('window'),type:'window',layerId:'windows',wallId:outer[0].id,t:.25,width:1200},{id:uid('dimension'),type:'dimension',layerId:'dimensions',wallId:outer[0].id,t1:0,t2:1,offset:-650},{id:uid('cadLine'),type:'cadLine',cadLayer:'AXIS',a:{x:4200,y:-900},b:{x:4200,y:6900},source:'sample DXF'});state.cadLayerVisibility=new Map([['0',true],['AXIS',true]]);state.cadMapping=null;state.sourceDxfName=null;state.sourceDxfMainBounds=null;state.sourceDxfFullBounds=null;state.sourceDxfOutlierCount=0;state.projectFileName=null;state.projectFileHandle=null;state.projectLocalKey=null;state.dirty=false;state.selectedObjectId=null;state.selectedObjectIds.clear();state.selectedReferenceId=null;rebuildObjectSnapIndex();switchToolset('plan',{skipMapping:true});updateAll();fitAll();}

  function activateCommand(command,tool,category,statusKey){setTool(tool,category);state.activeCommand=command;state.commandPending=null;state.trimPreview=null;state.spaceHoverPreview=null;dom.commandInput.blur();host.focus();recordEdit('command',{command,tool,toolset:state.toolset});setCommandStatus(t(statusKey),'strong');return true;}
  function runCommand(raw){
    const command=String(raw||'').trim().toUpperCase();
    if(!command){if(state.commandPending){setCommandStatus(t('command.finished'),'strong');state.commandPending=null;return;}if(state.lastCommand){return runCommand(state.lastCommand);}setCommandStatus(t('command.noPrevious'),'strong');dom.commandInput.focus();return;}
    if(state.commandPending==='zoom'){if(command==='E'||command==='EXTENTS'){fitAll();setCommandStatus(t('command.zoomExtents'),'strong');state.commandPending=null;dom.commandInput.blur();host.focus();return;}setCommandStatus(t('command.unknown',{command}),'error');return;}
    const plan=state.toolset==='plan';
    if(command==='L'||command==='LINE'){state.lastCommand='L';activateCommand('L','line','draw','command.line');}
    else if(command==='TR'||command==='TRIM'){state.lastCommand='TR';activateCommand('TR','trim','modify',plan?'command.trimPlan':'command.trim');}
    else if(command==='EX'||command==='EXTEND'){state.lastCommand='EX';activateCommand('EX','extend','modify',plan?'command.extendPlan':'command.extend');}
    else if(command==='E'||command==='ERASE'){state.lastCommand='E';if(selectionIds().size){deleteSelectedObjects();state.activeCommand=null;dom.commandInput.blur();host.focus();setCommandStatus('ERASE','strong');}else activateCommand('E','delete','modify','command.erase');}
    else if(command==='DI'||command==='DIST'){state.lastCommand='DI';activateCommand('DI','measure',plan?'select':'dimension','command.distance');}
    else if((command==='M'||command==='MOVE')&&plan){state.lastCommand='M';activateCommand('M','move','modify','command.move');}
    else if((command==='CO'||command==='COPY')&&plan){state.lastCommand='CO';activateCommand('CO','copy','modify','command.copy');}
    else if(command==='Z'||command==='ZOOM'){state.lastCommand='Z';state.activeCommand='Z';state.commandPending='zoom';dom.commandInput.blur();host.focus();setCommandStatus(t('command.zoom'),'strong');}
    else if(command==='U'||command==='UNDO'){state.lastCommand='U';state.activeCommand=null;undo();dom.commandInput.blur();host.focus();setCommandStatus(t('command.undo'),'strong');}
    else if(command==='REDO'){state.lastCommand='REDO';state.activeCommand=null;redo();dom.commandInput.blur();host.focus();setCommandStatus(t('command.redo'),'strong');}
    else if(command==='S'||command==='STRETCH'){setCommandStatus(t('command.stretchNotReady'),'error');}
    else if(command==='WALL'){state.lastCommand=plan?'L':'WALL';setTool(plan?'line':'wall',plan?'draw':'architecture');if(plan||state.cadMapping){state.activeCommand=plan?'L':'WALL';dom.commandInput.blur();host.focus();setCommandStatus(plan?t('command.line'):'WALL','strong');}}
    else if(command==='DOOR'){state.lastCommand='DOOR';setTool('door','architecture');if(plan||state.cadMapping){state.activeCommand='DOOR';dom.commandInput.blur();host.focus();setCommandStatus('DOOR','strong');}}
    else if(command==='WINDOW'){state.lastCommand='WINDOW';setTool('window','architecture');if(plan||state.cadMapping){state.activeCommand='WINDOW';dom.commandInput.blur();host.focus();setCommandStatus('WINDOW','strong');}}
  ...
  function dxfDoorEntities(obj,layer){
    ...
    else if(type==='hingedDouble'){emitLeaf(0,.5,'start');emitLeaf(.5,1,'end');}
    else if(type==='doubleActingSingle'){emitLeaf(0,1,obj.hinge||'start');emitLeaf(0,1,obj.hinge||'start',-doorSwingSide(obj));}
    else if(type==='doubleActingDouble'){for(const side of[doorSwingSide(obj),-doorSwingSide(obj)]){emitLeaf(0,.5,'start',side);emitLeaf(.5,1,'end',side);}}
  ...
  dom.commandInput.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();e.stopPropagation();const raw=dom.commandInput.value;dom.commandInput.value='';runCommand(raw);}else if(e.key==='Escape'){e.preventDefault();e.stopPropagation();dom.commandInput.value='';cancelTransient();host.focus();}});
  ...
  window.addEventListener('keydown',e=>{
    ...
    if(e.key==='Escape'){if(state.spaceGapDiagnostic){clearSpaceGapDiagnostic();return;}if(dom.exportSaveBackdrop&&!dom.exportSaveBackdrop.hidden){closeDxfFallbackSaveDialog();return;}if(!dom.confirmBackdrop.hidden){hideAppConfirm();return;}hideTooltip();hideContextMenu();closeFloorActionMenu();if(state.selectionDrag){state.selectionDrag=null;render();}else if(state.activeTool==='constraint'){cancelTransient();}else if(state.activeTool==='select'&&selectionIds().size){clearMultiSelection();renderPrimaryPanel();renderProperties();render();}else cancelTransient();}
  ...
  window.addEventListener('keyup',e=>{if(e.key==='Shift'){state.shiftDown=false;state.shiftGuide=null;render();}if(e.key==='Control'){state.ctrlDown=false;state.snapIndicator=null;render();}if(e.code==='Space'){const gesture=state.spaceGesture;state.spaceDown=false;state.spaceGesture=null;if(!state.pan)host.dataset.pan='false';if(gesture&&!gesture.used&&(performance.now()-gesture.started)<420&&state.view==='workspace'&&state.toolset==='plan'&&!isTyping()&&canvasShortcutContext())setTool('select','plan');}});

...
- 왼쪽 도구막대는 **선택 / 그리기 / 요소 / 수정 / 측정** 그룹으로 압축됩니다. `요소`에는 문·창·공간 지정이 들어가며 문은 여닫이·양개·미닫이 등 자주 쓰는 종류를 같은 그룹에서 고릅니다. 버튼을 누르면 마지막 사용 도구를 다시 실행하고 길게 누르기·우클릭·모서리 표시로 그룹을 펼칩니다.
- Plan Mode에서도 `L`, `TR`, `EX`, `E`, `DI`, Undo/Redo 같은 익숙한 명령을 사용할 수 있습니다. 이 명령은 원본 DXF가 아니라 현재 층의 Plan 객체를 편집합니다.
- 직선 선끼리 L/T 접합되면 단순히 같은 좌표에 놓는 데서 끝내지 않고 **지속적인 Junction 관계**를 유지합니다. 연결된 선의 끝점이나 선 자체를 움직이면 공유 접합점과 이웃 선이 함께 따라가거나 늘어나 접합이 쉽게 벌어지지 않습니다.
...
- 선택한 여닫이문은 **가운데 점을 끌어 선을 따라 위치 이동**합니다. 문 몸체를 선에 수직인 방향으로 끌면 열림 방향이 반전되고, 선을 따라 끌면 경첩 방향이 반전됩니다. 이 판단은 화면 수평/수직이 아니라 host 선의 로컬 축을 사용합니다.
